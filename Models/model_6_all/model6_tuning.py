...
def build_model(hp):
    ...
    for i in range(hp.Int("num_layers", 3, 5)):
        model.add(
            layers.Dense(
                ...
                units=hp.Int(f"units_{i}", min_value=200, max_value=8000, step=200),
    ...
    model.compile(
        optimizer="adam", loss="mae", metrics=[tf.keras.metrics.RootMeanSquaredError()],
    )
...
model_tuned.save("model_6")
model_tuned.save_weights("model_6_weights.h5")
